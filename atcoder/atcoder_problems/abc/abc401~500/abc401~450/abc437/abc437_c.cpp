...
void solve() {
    ...
    read(n);
    vi w(n), p(n);
    rep(i, n) read(w[i], p[i]);
    vi wp(n);
    rep(i, n) wp[i] = w[i] + p[i];
    sort(all(wp));
...
//     int N;
//     read(N);
//     vector<tuple<ll, ll, ll>> deer(N + 1);
//     rep(i, N) {
//         ll w, p;
//         read(w, p);
//         deer[i] = mt(p-w, p, w);
...
//     sort(all(deer));
//     rep(i, N) {
//         get<1>(deer[i + 1]) = get<1>(deer[i]) + get<1>(deer[i + 1]);
//         get<2>(deer[i + 1]) = get<2>(deer[i]) + get<2>(deer[i + 1]);
//     }

//     int ok = 0, ng = N;
//     while (abs(ng - ok) > 1) {
//         int mid = (ok + ng) / 2;
//         ll power = get<1>(deer[N]) - get<1>(deer[mid]), weight = get<2>(deer[mid]) - get<2>(deer[0]);
//         if (power >= weight) ok = mid;
//         else ng = mid;
//     }
//     print(ok);
// }
