...
//     rep(n, N) {
//         vll nxt(MAX);
//         int cur_idx = T.find(S[n]);
        
//         rep(i, MAX) {
//             if (dp[i] == 0) continue;
//             rep(j, 3) {
//                 if (!(i >> cur_idx & 1) && !(i >> j & 1)) continue;
//                 nxt[(1 << cur_idx) | (1 << j)] = (nxt[(1 << cur_idx) | (1 << j)] + dp[i]) % MOD;    
//             }
//             nxt[MAX - 1] = (nxt[MAX - 1] + dp[i]) % MOD;
//         }
//         swap(dp, nxt);
//     }

//     ll ans = 0;
//     rep(i, MAX) ans = (ans + dp[i]) % MOD;
//     cout << ans << endl;
//     return 0;
// }